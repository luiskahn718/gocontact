...
RSpec.describe PhoneNumbersController, type: :controller do
  ...
  let(:valid_attributes) { { number: '1112223333', person_id: person.id } }
  ...
  describe "PUT #update" do
    context "with valid params" do
      let(:new_attributes) { { number: '1112223333', person: person } }

      it "updates the requested phone_number" do
        phone_number = PhoneNumber.create! valid_attributes
        put :update, {:id => phone_number.to_param, :phone_number => new_attributes}, valid_session
        phone_number.reload
        expect(phone_number.number).to eq('1112223333')
